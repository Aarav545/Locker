...
const client = new cassandra.Client({
    cloud: { secureConnectBundle: '../../driver/secure-connect-bucket.zip' },
    credentials: { username: 'nabeel', password: 'X6-]7Pzq^B9>&_L@' },
    keyspace: 'data'
  });

function createTable(){
    const query = "CREATE TABLE IF NOT EXISTS data.users \
    (lastname text PRIMARY KEY, email text, firstname text);";
    return client.execute(query);
  }

function insertUser(lastname, email, firstname) {
    // TO DO: execute a simple statement that inserts one user into the table
    const insert = 'INSERT INTO users (lastname, email, firstname) VALUES (?,?,?,?,?)';
    const params = [ lastname, email, firstname ];
