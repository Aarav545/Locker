...
export default function handler (req, res) {
  if (req.method === 'POST') {
    ...
    if (req.body.googleId) {
      const user = req.body
      if (user.googleId) {
        _shareNotes(res, user.partner, user.noteTitle, user.noteContent)
      }
    }
  } else {
...
function _shareNotes (res, partner, noteTitle, noteDesc) {
  col.find({ noteTitle })
    .then(() => {
      users.findOne({ email: partner }).then(response => {
        if (response.id) {
          col.insert({
            id: response.id,
            email: partner,
